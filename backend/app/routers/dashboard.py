...
# ─── GET /dashboard/nav-sync-status ──────────────────────────────────────────

@router.get("/nav-sync-status")
def get_nav_sync_status():
    """
    Returns the configuration state of the NAV data sync engine.
    Indicates whether Alibaba Cloud OSS sync is enabled or local scraper is used.
    """
    from app.services.oss_nav_sync import get_sync_status
    return get_sync_status()


# ─── POST /dashboard/sync-navs ────────────────────────────────────────────────

@router.post("/sync-navs")
def trigger_nav_sync(db: Session = Depends(database.get_db)):
    """
    Triggers a manual NAV sync:
      1. Tries Alibaba Cloud OSS first (200ms, no browser).
      2. Falls back to local Playwright scraper if cloud data is stale or offline.

    Returns the sync result including source and fund count updated.
    """
    import threading
    from app.services.oss_nav_sync import sync_nav_data

    def _bg_sync():
        try:
            sync_result = sync_nav_data(db)
            import logging
            logging.getLogger(__name__).info(f"NAV sync complete: {sync_result}")
        except Exception as e:
            import logging
            logging.getLogger(__name__).error(f"NAV sync failed: {e}")

    t = threading.Thread(target=_bg_sync, daemon=True)
    t.start()
    return {"status": "triggered", "message": "NAV sync started in background. Cloud OSS checked first, Playwright fallback if needed."}
